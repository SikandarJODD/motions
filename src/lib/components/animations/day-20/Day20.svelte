<script>
  ...
  import GrayWrapper from "./GrayWrapper.svelte";
  let inputText = "svelte is best framework to build interactive web apps";
  let isHover = false;
  let onMouseEnter = () => (isHover = true);
  let onMouseLeave = () => (isHover = false);
</script>

<!-- svelte-ignore a11y-no-static-element-interactions -->
<div
  class="relative flex h-[388px] w-full max-w-[333px] flex-col items-end gap-6 overflow-hidden rounded-[10px] border border-[#262626] bg-[#181818] py-7"
  on:mouseenter={onMouseEnter}
  on:mouseleave={onMouseLeave}
>
  <!-- Right Shadow -->
  <div
    class="absolute bottom-0 right-0 top-0 z-10 w-1/2 bg-gradient-to-r from-transparent to-[#181818]"
  ></div>
  <!-- Original Component -->
  <MotionConfig transition={{ duration: 1, type: "spring", bounce: 0 }}>
    <GrayWrapper {isHover}>
      ...
      <div>
        <h3 class="text-sm font-medium tracking-tight text-[#3D3C3A]">Info!</h3>
        <p class="text-xs text-[#3D3C3A]">
    ...
    <!-- Second -->
    <div
      class="relative flex h-16 w-72 items-center gap-4 rounded-lg border-[#A66942] bg-[#5D331D] pl-4"
    >
      <svg
        width="31"
        height="27"
      ...
      >
        ...
        <defs>
          ...
          >
            ...
            <stop offset="1" stop-color="#E08238" />
          </linearGradient>
        </defs>
      </svg>
      <div>
        <h3 class="text-sm font-medium tracking-tight text-[#C26F35]">
          Warning!
        </h3>
        <p class="text-xs text-[#946A52]">Maximum callstack exceeded</p>
      </div>
      ...
      >
        <div
          use:motion
          class="absolute left-0 h-[88px] w-[90%] origin-top-left rounded-sm bg-[#5D331D] mix-blend-color-dodge"
        >
          <svg
            viewBox="0 0 18 18"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            class="absolute bottom-0 right-0 w-3 translate-x-full translate-y-full"
          >
            <path
    ...
    <!-- Input  -->
    <div
      class="mt-auto flex h-11 w-80 items-center gap-4 rounded-lg border-[#A66942] bg-[#5D331D] pl-4"
    >
      <svg
        width="17"
        height="16"
      ...
      <p class="text-nowrap -tracking-4 text-sm text-[#946A52]">
        {#each inputText.split("") as item, index (index)}
          <Motion
      ...
      <div
        class="h-[35%] w-px -translate-x-4 animate-pulse rounded-full bg-white/50"
      ></div>
    </div>
  </MotionConfig>
</div>
